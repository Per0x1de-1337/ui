...
const WecsTreeview = () => {
  ...
  const getTimeAgo = useCallback((timestamp: string | undefined): string => {
    console.log("[WecsTopology] getTimeAgo called with timestamp:", timestamp);
  ...
  useEffect(() => {
    if (wecsData !== null && !isEqual(wecsData, prevWecsData.current)) {
      console.log("[WecsTopology] wecsData:", wecsData); // Log the wecsData object
